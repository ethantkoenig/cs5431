...
This serves as a countermeasure against session-hijacking attacks, and leaks of a user's authentication secret.
...
In the web app, we allow users to authorize which other users can and cannot send them money.
...
The web-app maintains a log of all system ``events'' that pertain to our security requirements. This log can be manually reviewed to detect and diagnose any attacks or misconduct.
...
Our web-app has a number of safeguards previously mentioned to protect user confidentiality. While users who are logged in can see all the usernames of other users, they learn nothing else about those users. Users' keys, email, balance, friends are not disclosed to other users.
...
If an attacker alters the public keys that are stored under a particular user, then anyone paying that user may inadvertently pay the attacker's public key. We protect against such attacks through two-factor authentication for key addition.
